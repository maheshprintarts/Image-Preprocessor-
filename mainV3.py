    ...
    try:
        ...
        if img_obj:
            with img_obj as img:
                ...
                # Perceptual Zoom Target (Matches user's 10% UI zoom preference)
                target_scale = 0.10
                
                final_w = int(img.width * target_scale)
                final_h = int(img.height * target_scale)
                
                print(f"Applying Target Zoom Scale (10%): Shrinking {img.width}x{img.height} -> {final_w}x{final_h}")
                img = progressive_resize(img, (final_w, final_h))
                
                # Save Final Output
                if output_path.lower().endswith(('.jpg', '.jpeg')):
                    final_img = img.convert('RGB')
                    # User explicitly requested native 96 target resolution format
                    final_img.save(output_path, dpi=(96, 96), quality=100, subsampling=0, optimize=True, progressive=True)
                else:
                    final_img = img  # Keep RGBA for PNG
                    final_img.save(output_path, dpi=(96, 96))
                
                print(f"Saved optimized image to: {output_path} (Size: {final_img.size[0]}x{final_img.size[1]} @ 96 DPI)\n")
